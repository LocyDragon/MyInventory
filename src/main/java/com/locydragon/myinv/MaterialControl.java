...
/**
 * from: https://raw.githubusercontent.com/25/EnchantmentsEnhance/master/EnchantmentsEnhance-Core/src/main/java/org/pixeltime/enchantmentsenhance/util/XMaterial.java
 * from: https://github.com/Bkm016/TabooMenu/blob/master/src/me/skymc/taboomenu/version/MaterialControl.java
 */
public enum MaterialControl {
    ...
    public static String[] collect(boolean newVersion) {
        ...
        return Arrays.stream(values).map(value -> newVersion ? value.name() : value.m).toArray(String[]::new);
    }
    ...
    public static MaterialControl requestXMaterial(String name, byte data) {
        if (cachedSearch.containsKey(name.toUpperCase() + "," + data)) {
            return cachedSearch.get(name.toUpperCase() + "," + data);
        }
        for (MaterialControl mat : MaterialControl.values()) {
    ...
    public static MaterialControl fromString(String key) {
        ...
        try {
            material = MaterialControl.valueOf(key);
            return material;
        } catch (IllegalArgumentException e) {
            String[] split = key.split(":");
            if (split.length == 1) {
                material = requestXMaterial(key, (byte) 0);
            } else {
                material = requestXMaterial(split[0], (byte) Integer.parseInt(split[1]));
            }
            return material;
        }
    }
    ...
    public boolean isSameMaterial(ItemStack comp) {
        ...
        MaterialControl material = fromMaterial(comp.getType());
        if (isDamageable(material)) {
    ...
    public MaterialControl fromMaterial(Material mat) {
        try {
            ...
            for (MaterialControl material : MaterialControl.values()) {
                if (material.m.equals(mat.toString())) {
                    return material;
                }
            }
        }
